...
def addCurrency(
        ...
        units_per_coin,
        # Return to the root of the crypto repo (not the '-gui' one!) and go to <NAME> -> consensus -> block_rewards.py. You should see a variable named _mojo_per_<NAME> (_mojo_per_flax, _mojo_per_chia - mojo might be replaced with a different name) set to a high number (usually 1000000000000). Copy that number and put it here (digits only)
        min_fee,
        # From my understanding, there's no minimum fee, so you can just set this to 0. However, setting the fee to 1 mojo MIGHT give the transactions higher priority and lead to better confirmation times. Keep in mind that the fee is paid two times: once when the contract is created and one time when the contract is claimed.
        default_max_block_height,
        # This should be set to the approximate average number of blocks added in a 24h period. Go to <NAME> -> consensus -> default_constants.py and look for the line where "EPOCH_BLOCKS" is set.
        default_min_confirmation_height,
        # This value is little bit subjective and depends on the stability of the network. Recommended minimum is 25.
...
def guessSslDirFor(currencyName):
    return os.path.join(str(Path.home()),
                        f".{currencyName.lower()}{'/' if '/' in currencyName else '/mainnet'}/config/ssl")
