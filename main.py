...
class Trades(Resource):
    def get(self):
        ...
        for row in result:
            ...
            trade_currency_one = conn.execute(stmt).fetchall()[0]
            stmt = trade_currencies.select().where(trade_currencies.c.id == row[2])
            trade_currency_two = conn.execute(stmt).fetchall()[0]

            res.append(tradesRowToJson(row, tradeCurrencyRowToJson(trade_currency_one),
                                       tradeCurrencyRowToJson(trade_currency_two)))
...
def tradeWaitForContract(trade_index, trade, trade_currency, currency, issue_contract, wait=False,
                         other_trade_currency=False, other_currency=False, checkFunc=False):
    ...
    trade_threads_files[trade_index].write(
        f"Waiting for contract with puzzlehash {programPuzzleHash} and address {programAddress} to be confirmed\n")
    ...
    if issue_contract:
        trade_threads_messages[
            trade_index] = f"Please send {amount_to_send:.12f} {currency.name} with a fee of {fee:.12f} {currency.name} to the address found below. Double-check the address before confirming the transaction - if it's wrong, your coins will be lost."
        trade_threads_addresses[trade_index] = programAddress
    else:
        trade_threads_messages[
            trade_index] = f"Waiting for the other human to send {amount_to_send:.12f} {currency.name} with a fee of {fee:.12f} {currency.name} to the address found below..."
        trade_threads_addresses[trade_index] = programAddress
    ...
    if other_trade_currency != False and other_currency != False:
        ...
        other_coin_record = other_full_node_client.getContractCoinRecord(otherProgramPuzzleHash.hex(),
                                                                         height - 1000 - other_trade_currency.max_block_height)
        ...
        trade_threads_files[trade_index].write(
            f"Other coin record: {other_coin_record}\nShould cancel? {shouldCancel}\n")
        trade_threads_files[trade_index].flush()

    contract_coin_record = full_node_client.getContractCoinRecord(programPuzzleHash.hex(),
                                                                  height - 1000 - trade_currency.max_block_height)
    while contract_coin_record == False and shouldCancel == False:
        ...
        if other_trade_currency != False:
            ...
            if other_height - other_coin_block_index >= other_trade_currency.max_block_height * 3 // 4 - ceil(
                    trade_currency.min_confirmation_height * trade_currency.max_block_height / other_trade_currency.max_block_height):
                shouldCancel = True
    ...
    if shouldCancel == False and contract_coin_record["coin"][
        "amount"] != trade_currency.total_amount - trade_currency.fee:
    ...
    if shouldCancel:
        ...
        while confirmed_block_index + trade_currency.min_confirmation_height > height:
            ...
            trade_threads_messages[
                trade_index] = f"Waiting for transaction confirmation ({delta} / {trade_currency.min_confirmation_height})"
            trade_threads_addresses[trade_index] = None
            time.sleep(10)
            height = full_node_client.getBlockchainHeight()
...
def lookForSolutionInBlockchain(trade_index, trade, trade_currency, currency, coin_record, other_trade_currency=False,
    ...
    trade_threads_files[trade_index].write(
        f"Loking for solution of contract with puzzlehash {programPuzzleHash}\nKeeping an eye on {otherProgramPuzzleHash}\n")
    trade_threads_files[trade_index].flush()
    ...
    if coin_record == False:
        ...
        height = full_node_client.getBlockchainHeight()
        coin_record = full_node_client.getContractCoinRecord(programPuzzleHash,
                                                             height - 1000 - trade_currency.max_block_height, True)
    ...
    if other_full_node_client != False:
        other_height = other_full_node_client.getBlockchainHeight()
        other_coin_record = other_full_node_client.getContractCoinRecord(otherProgramPuzzleHash,
                                                                         other_height - 1000 - other_trade_currency.max_block_height,
                                                                         True)
    ...
    while spent_block_index == 0:
        ...
        if other_full_node_client != False:
            other_height = other_full_node_client.getBlockchainHeight()
            if other_height - other_coin_record[
                'confirmed_block_index'] >= other_trade_currency.max_block_height * 3 // 4:
    ...
    coin_id = std_hash(
        bytes.fromhex(coin["parent_coin_info"][2:]) + bytes.fromhex(coin["puzzle_hash"][2:]) + int_to_bytes(
            coin["amount"])).hex()
...
def tradeClaimContract(trade_index, trade, trade_currency, currency, solution_program_hex, coin_record, cancel=False):
    ...
    if coin_record == False:
        ...
        height = full_node_client.getBlockchainHeight()
        coin_record = full_node_client.getContractCoinRecord(programPuzzleHash,
                                                             height - 10000 - trade_currency.max_block_height, True)
    trade_threads_files[trade_index].write(f"Coin record: {coin_record}\n")
    trade_threads_files[trade_index].flush()
    ...
    if r == "pending":
        while r == "pending":
            trade_threads_messages[
                trade_index] = "The transaction was marked as PENDING - I'll push it every 30 seconds just to be sure"
            r = full_node_client.pushTransaction(
                program.as_bin().hex(),
                solution_program_hex,
                coin
...
def tradeCode(trade_id):
    ...
    trade = conn.execute(s).fetchall()[0]
    ...
    trade_currency_one = conn.execute(s).fetchall()[0]
    ...
    currency_one = conn.execute(s).fetchall()[0]

    s = trade_currencies.select().where(trade_currencies.c.id == trade.trade_currency_two)
    trade_currency_two = conn.execute(s).fetchall()[0]
    ...
    currency_two = conn.execute(s).fetchall()[0]
    ...
    if trade.step == 0:
        shouldCancel, coin_record_one = tradeWaitForContract(trade_index, trade, trade_currency_one, currency_one,
                                                             trade.is_buyer, True)
    ...
    if trade.step == 1:
        shouldCancel, coin_record_two = tradeWaitForContract(trade_index, trade, trade_currency_two, currency_two,
                                                             not trade.is_buyer, coming_from_step_0, trade_currency_one,
                                                             currency_one)
    ...
    if trade.step == 2:
        ...
        if not cancelTrade:
            if trade.is_buyer:
                coin_record_two, cancelTrade = shouldCancelTrade(trade_index, trade, trade_currency_two, currency_two,
                                                                 coin_record_two)
            else:
                coin_record_one, cancelTrade = shouldCancelTrade(trade_index, trade, trade_currency_one, currency_one,
                                                                 coin_record_one)

        trade_threads_files[trade_index].write(f"Cancel trade: {cancelTrade}\n")
        trade_threads_files[trade_index].flush()
        if cancelTrade:
            solution_program = getSolutionProgram(
                "CANCEL-" + str(random.SystemRandom().getrandbits(128))).as_bin().hex()
            if trade.is_buyer:
                tradeClaimContract(trade_index, trade, trade_currency_one, currency_one, solution_program,
                                   coin_record_one, True)
            else:
                tradeClaimContract(trade_index, trade, trade_currency_two, currency_two, solution_program,
                                   coin_record_two, True)
        else:
            if trade.is_buyer:
                ...
                solution_program = lookForSolutionInBlockchain(trade_index, trade, trade_currency_two, currency_two,
                                                               coin_record_two, trade_currency_one, currency_one)
...
class EthTrades(Resource):
    def get(self):
        ...
        for row in result:
            stmt = trade_currencies.select().where(trade_currencies.c.id == row[1])
            trade_currency = conn.execute(stmt).fetchall()[0]
...
def ethTradeCode(trade_id):
    ...
    trade_currency = conn.execute(s).fetchall()[0]
    ...
    currency = conn.execute(s).fetchall()[0]
    ...
    if trade.step == 0:
        ...
        if trade.is_buyer:
            trade_threads_messages[
                trade_index] = f"Press the button below to create the swap on the Ethereum blockchain\nMake sure you're connected to the following network: {trade[9]}"
        ...
        trade_threads_messages[
            trade_index] = f"Waiting for swap to be confirmed on the Ethereum blockchain...\nMake sure you're connected to the following network: {trade[9]}"
        ...
        while confirmations < ETH_REQUIRED_CONFIRMATIONS:
            trade_threads_messages[
                trade_index] = f"Confirming swap creation ({confirmations}/{ETH_REQUIRED_CONFIRMATIONS})"
            confirmations = getResponse(trade_id, "confirmations")
            time.sleep(5)
    ...
    if trade.step == 1:
        ...
        if not shouldCancel:
            ...
            shouldCancel, coin_record = tradeWaitForContract(trade_index, trade, trade_currency, currency,
                                                             not trade.is_buyer, False, False, False, checkFunc)
    ...
    if trade.step == 2:
        ...
        if not cancelTrade:
            ...
            trade_threads_commands[trade_index] = None
            cancelTrade = cancelTrade or (
                        eth_trade_responses[trade_id]['confirmations'] > ETH_MAX_BLOCK_HEIGHT * 3 // 4)
        ...
        if cancelTrade:
            ...
            if not trade.is_buyer:
                ...
                while eth_trade_responses[trade_id]["confirmations"] < ETH_MAX_BLOCK_HEIGHT:
                    trade_threads_messages[
                        trade_index] = f"{ETH_MAX_BLOCK_HEIGHT - eth_trade_responses[trade_id]['confirmations']} blocks left before you can cancel the swap..."
                    trade_threads_commands[trade_index] = {"code": "WAIT_FOR_SWAP", "args": swap_data}
            ...
            if not trade.is_buyer:
                ...
                trade_threads_commands[trade_index] = None
                solution_program = lookForSolutionInBlockchain(trade_index, trade, trade_currency, currency,
                                                               coin_record)
